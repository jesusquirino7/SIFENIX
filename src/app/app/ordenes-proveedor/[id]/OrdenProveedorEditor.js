...
const EMPTY_ROW = { part_number: "", manufacturer: "", description: "", quantity: 1, unit_cost: "" };

export default function OrdenProveedorEditor({ order, items, editable }) {
  ...
  async function handleSubmit(e) {
    ...
    await supabase.from("supplier_order_items").delete().eq("supplier_order_id", order.id);
        ...
        <div className="mt-2 space-y-3">
          {rows.map((row, index) => (
            ...
            >
              ...
              <MoneyInput
                value={row.unit_cost}
                onChange={(v) => updateRow(index, "unit_cost", v)}
