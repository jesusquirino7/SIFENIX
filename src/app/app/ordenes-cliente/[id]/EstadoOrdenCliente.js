...
export default function EstadoOrdenCliente({ orderId, status }) {
  const router = useRouter();
  const { showToast } = useToast();
  const [saving, setSaving] = useState(false);

  const current = CUSTOMER_ORDER_STATUS[status] || CUSTOMER_ORDER_STATUS.confirmed;
  const nextOptions = CUSTOMER_ORDER_STATUS_FLOW[status] || [];
  ...
  async function changeStatus(nextStatus) {
    ...
    showToast(
      `Orden marcada como ${CUSTOMER_ORDER_STATUS[nextStatus].label.toLowerCase()}.`
    );
    router.refresh();
  }
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      {nextOptions.map((next) => (
        ...
        >
          Marcar como {CUSTOMER_ORDER_STATUS[next].label.toLowerCase()}
        </button>
      ))}
    </div>
  );
