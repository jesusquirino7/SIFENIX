...
Además del sitio público, el repo incluye un sistema privado de gestión (CRM/ERP ligero) para la operación de la distribuidora, sobre el mismo proyecto de Next.js/Vercel (no es un repo ni proyecto aparte). Se está construyendo en la rama `feature/sistema-interno` — no está mergeado a `master`/producción todavía, por decisión explícita del dueño de no tocar el sitio público mientras se construye.

### Arquitectura
- El sitio público se movió a `src/app/(public)/` (route group — no cambia ninguna URL) con su propio `layout.js` (el Header/Footer de siempre). El `layout.js` raíz (`src/app/layout.js`) quedó mínimo (solo `<html>/<body>` + metadata) para que `/login` y `/app/*` no lleven la navegación pública.
...
### Supabase
Proyecto: `oazrvmuxupxxbeeboplq` (`https://oazrvmuxupxxbeeboplq.supabase.co`). Credenciales en `.env.local` (no versionado, `.gitignore` cubre `.env*`) y ya cargadas en Vercel (Production/Preview/Development) vía `vercel env add` — no hace falta tocarlas a mano en el dashboard de Vercel.
- `src/lib/supabase/client.js` — cliente para Client Components. `src/lib/supabase/server.js` — cliente para Server Components/Route Handlers (usa `cookies()`).
- Esquema versionado en `supabase/migrations/` (se aplican a mano, pegando el SQL en el SQL Editor de Supabase — no hay Supabase CLI conectado todavía, ver `supabase/README.md`). `0001_sistema_interno_inicial.sql` ya está corrida en el proyecto real: crea `profiles`, `customers`, `customer_contacts`, `suppliers`, `supplier_contacts`, `products`, `opportunities`, `opportunity_items` y `audit_log` (esqueleto, sin triggers conectados todavía), con Row Level Security en todas — política actual: cualquier usuario con `profiles.active = true` tiene acceso completo de lectura/escritura, sin distinción por `role` (eso es trabajo de una etapa posterior, reemplazando policies sin tocar el esquema).
- `profiles.role` puede ser `super_admin`, `admin`, `sales`, `purchasing` u `operations`. Se crea automáticamente (`role` default `'sales'`) al registrarse un usuario en Supabase Auth (trigger `handle_new_user`); el primer `super_admin` se promovió a mano desde el Table Editor.

### Módulos ya funcionales (Etapa 1 del plan del sistema interno — validada por el dueño con datos reales)
- **Dashboard** (`/app/dashboard`): tarjetas con conteos reales (oportunidades activas, clientes/proveedores activos) + sección "Requiere atención" con estado vacío honesto (se activa cuando existan cotizaciones/RFQ/órdenes, Etapa 2 en adelante — no se inventan datos).
- **Clientes** (`/app/clientes`), **Proveedores** (`/app/proveedores`) y **Productos** (`/app/productos`): mismo patrón (lista + alta vía modal + detalle/edición en página propia), cada uno con su propio `<Entidad>Form.js` reutilizado entre alta y edición.
- **Oportunidades** (`/app/oportunidades`): el concepto central del sistema — cada oportunidad (`opportunity_number` autogenerado tipo `OP-2026-00001`, función `generate_opportunity_number` en la migración) liga un cliente con los productos que pidió cotizar (`opportunity_items`, capturados como lista dinámica en `/app/oportunidades/nueva`). El detalle (`/app/oportunidades/[id]`) ya trae una sección "Documentos relacionados" preparada para mostrar, en etapas futuras, la cotización, RFQs, comparativo y órdenes de esa operación.
- Resto del sidebar (Cotizaciones, RFQ Proveedores, Órdenes de Cliente/Proveedor, Seguimiento, Reportes) son páginas "Próximamente" (`src/components/app/ComingSoonPage.js`) — navegación ya lista, contenido pendiente de las etapas 2 en adelante.
- `/app/configuracion` por ahora solo muestra el perfil del usuario actual (nombre, correo, rol) — gestión de usuarios/permisos por rol queda para una etapa posterior; mientras tanto los roles se asignan a mano en el Table Editor de Supabase.

### Plan del sistema interno (independiente del plan de lanzamiento del sitio público de abajo)
Etapa 1 (arriba) completa y validada. Sigue: Etapa 2 Cotizaciones a clientes → Etapa 3 RFQ y cotizaciones de proveedores → Etapa 4 Comparativo de proveedores → Etapa 5 Órdenes de compra de clientes → Etapa 6 Órdenes de compra a proveedores → Etapa 7 Seguimiento y alertas → Etapa 8 Dashboard avanzado/rentabilidad → Etapa 9 Automatización e IA. Pendiente decidir con el dueño cuándo mergear `feature/sistema-interno` a `master` vs. seguir construyendo etapas ahí primero.
